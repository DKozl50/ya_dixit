...
def one_hand(player):
    ...
    print(f'|-player {player} cards:-|' + ('<-leader' if player == game.current_player else ''))
...
for p in game.players:
    game._deal6(p)
game.current_player = game.players[game.state]
# MAGIC ENDS HERE 
#
demonstrate_hands()
print('res:', game.result)
...
print('Start turn giving association and choosing card', card_from(game.current_player))
guessed_card = card_from(game.current_player)  # stored for demonstration purposes
game.start_turn('association 1', card_from(game.current_player))
demonstrate_hands()
print()

print('Other players give cards:')
# I assume cards are given simultaneously
cards_on_table = {p: card_from(p) for p in [1, 2, 3] if p != game.current_player}  # stored for demonstration purposes
game.place_cards(cards_on_table)
cards_on_table[game.current_player] = guessed_card  # stored for demonstration purposes
demonstrate_hands()
print('res:', game.result)
print()
...
game.valuate_guesses({p: cards_on_table[p % 3 + 1] for p in [1, 2, 3] if p != game.current_player})  # everyone voted for the next player
print('res:', game.result)
print()
...
guessed_card = card_from(game.current_player)
game.start_turn('association 2', card_from(game.current_player))
# stuff is stored again for demonstration purposes
cards_on_table = {p: card_from(p) for p in [1, 2, 3, 4] if p != game.current_player}
game.place_cards(cards_on_table)
cards_on_table[game.current_player] = guessed_card
# score can never get <0
game.valuate_guesses({p: guessed_card for p in [1, 2, 3, 4] if p != game.current_player})
game.end_turn()
demonstrate_hands()
print('res:', game.result)
print()
...
guessed_card = card_from(game.current_player)
game.start_turn('association 3', card_from(game.current_player))
# someone may pass their move
cards_on_table = {p: card_from(p) for p in [1, 2, 3, 4] if p != game.current_player}
cards_on_table.pop(game.current_player % 4 + 1)
game.place_cards(cards_on_table)
demonstrate_hands()
print()
